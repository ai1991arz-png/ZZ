...
name: llm-council
description: >
  Прогони решение через совет из 5 ИИ-советников: независимый разбор
  → председатель находит зерно и сводит лучшие идеи в усиленную рекомендацию.
  Режимы: экспресс (по умолчанию) и полный (с перекрёстным обзором).
  ТРИГГЕРЫ: «спроси совет», «созови совет», «council this».
  СИЛЬНЫЕ (когда есть размен): «X или Y», «не могу решить», «разрываюсь между», «провалидируй».
  ПОЛНЫЙ: «полный совет», «стресс-тест».
  ВТОРОЙ РАУНД: возражение → переигрываем только спорную точку.
  ВИЗУАЛ: «визуал», «инфографика».
  НЕ срабатывает на простые да/нет, факты и вопросы без реального размена.
---
...
**A. Просканируй рабочее пространство на контекст.** Вопрос пользователя обычно — лишь верхушка айсберга. В его установке, скорее всего, есть файлы, которые драматически улучшат вывод совета. Перед формулировкой быстро поищи и прочитай релевантные файлы:
...
- **Зерно** — одна-две фразы. Самое важное, что вытащили советники. Если на сессии случилось озарение — оно стоит здесь. Не повтор очевидного.
- **Лучшие идеи** (с возможным усилением) — топ-2 или топ-3 идеи из ответов. Где председатель видит, что идея одного советника становится сильнее, если соединить её с инсайтом другого, — он явно показывает: «Идея X от Оптимиста становится прочнее, если пропустить её через ограничение, которое заметил Критик. Получается: …». Если уместно — таблица сравнения вариантов.
- **Полезные разногласия** — где советники разошлись и почему это важный размен, а не просто шум. Не сглаживать, но и не драматизировать. Это карта реальных tradeoffs, на которые пользователь должен посмотреть осознанно.
- **Рекомендация** — чёткая, усиленная, реализуемая рекомендация. Не «зависит от ситуации». Не «учитывай обе стороны». Реальный ответ с обоснованием.
- **С чего начать** — один конкретный следующий шаг. Не список из 10 пунктов. Одна вещь.
...
Сгенерируй визуальную сводку итога. Если доступен инструмент визуализации — используй его. Иначе — создай SVG-файл с инфографикой.
